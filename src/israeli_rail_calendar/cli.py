...
from israeli_rail_calendar.calendar_generator import (
    create_event,
    format_duration,
...
async def process_day_for_route(
    ...
    for attempt_days in lookahead_days:
        ...
        try:
            return await get_train_schedule(
                session, origin_id, dest_id, date_str, semaphore
            )
        except Exception as e:
            logging.warning(
                f"Failed to fetch for {date_str} with {attempt_days} days fallback: {e}"
            )
            if attempt_days == 4:
                logging.error(
                    f"Critical error: Could not fetch schedules for {date_str} even at 4 days lookahead."
                )
    return None
...
async def process_route(
    ...
    cal_first_last["prodid"] = vText(
        "-//Israel Railways Calendar Generator (First/Last)//"
    )
    cal_first_last["version"] = vText("2.0")
    cal_first_last["X-WR-CALNAME"] = vText(
        f"{route.origin} to {route.destination} (First & Last Trains)"
    )

    cal_all = Calendar()
    cal_all["prodid"] = vText("-//Israel Railways Calendar Generator (All Trains)//")
    cal_all["version"] = vText("2.0")
    cal_all["X-WR-CALNAME"] = vText(
        f"{route.origin} to {route.destination} (All Trains)"
    )

    today = datetime.date.today()
    update_time = datetime.datetime.now(pytz.timezone("Asia/Jerusalem")).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    for routes_data in results:
        ...
        for i, train_route in enumerate(routes_data):
            ...
            cal_all.add_component(
                create_event(
                    title, dep, description=event_desc, html_description=html_desc
                )
            )
        ...
        first_desc, first_html_desc = generate_event_description(
            first_train, update_time
        )
        cal_first_last.add_component(
            create_event(
                first_title,
                first_dep,
                description=first_desc,
                html_description=first_html_desc,
            )
        )
        ...
        cal_first_last.add_component(
            create_event(
                ...
                html_description=last_html_desc,
            )
        )
...
def get_active_routes() -> List[RouteConfig]:
    ...
    active_stations = [
        name for name, is_active in config_model.root.items() if is_active
    ]
...
async def main_async(output_dir: Path):
    ...
    async with aiohttp.ClientSession(headers=HEADERS, connector=connector) as session:
        tasks = [
            process_route(route, output_dir, session, semaphore) for route in routes
        ]
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="calendars", help="Directory to save the .ics files"
    )
