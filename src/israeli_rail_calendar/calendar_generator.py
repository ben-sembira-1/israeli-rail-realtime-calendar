...
def generate_event_description(
    train_route: TrainRouteModel, update_time: str
) -> Tuple[str, str]:
    text_lines: List[str] = []
    html_lines: List[str] = []
    ...
    for train in train_route.trains:
        ...
        leg_dest = ID_TO_STATION.get(
            str(train.destinationStation), str(train.destinationStation)
        )
        ...
        dep_time = (
            train.departureTime.split("T")[1][:5]
            if "T" in train.departureTime
            else train.departureTime
        )
        orig_plat = (
            f" (Platform {train.originPlatform})"
            if train.originPlatform is not None
            else ""
        )
        ...
        for stop in train.stopStations:
            ...
            arr_time = (
                stop.arrivalTime.split("T")[1][:5]
                if "T" in stop.arrivalTime
                else stop.arrivalTime
            )
            stop_plat = (
                f" (Platform {stop.platform})" if stop.platform is not None else ""
            )
        ...
        dest_arr_time = (
            train.arrivalTime.split("T")[1][:5]
            if "T" in train.arrivalTime
            else train.arrivalTime
        )
        dest_plat = (
            f" (Platform {train.destPlatform})"
            if train.destPlatform is not None
            else ""
        )
