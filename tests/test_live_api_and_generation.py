...
async def test_live_api_and_generation(tmp_path: Path):
    ...
    assert (
        config_file.exists()
    ), "stations_config.json should exist in the repository root"

    # We load active routes based on the config file
    routes = get_active_routes()
    assert (
        len(routes) > 0
    ), "There should be at least one active route generated from stations_config.json"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [
            process_route(route, calendars_dir, session, semaphore)
        ...
        for route in test_routes:
            base_filename = (
                route.filename[:-4]
                if route.filename.endswith(".ics")
                else route.filename
            )

            expected_first_last = calendars_dir / f"{base_filename}.first_last.ics"
            assert (
                expected_first_last.exists()
            ), f"Expected file {expected_first_last.name} was not created"

            content_fl = expected_first_last.read_text()
            assert (
                "BEGIN:VCALENDAR" in content_fl
            ), "ICS file must contain BEGIN:VCALENDAR"
            assert "DTSTART" in content_fl, "ICS file must contain DTSTART"
            assert (
                "Asia/Jerusalem" in content_fl
            ), "ICS file must specify Asia/Jerusalem timezone"

            expected_all = calendars_dir / f"{base_filename}.all.ics"
            assert (
                expected_all.exists()
            ), f"Expected file {expected_all.name} was not created"

            content_all = expected_all.read_text()
            assert (
                "BEGIN:VCALENDAR" in content_all
            ), "ICS file must contain BEGIN:VCALENDAR"
            assert "DTSTART" in content_all, "ICS file must contain DTSTART"
            assert (
                "Asia/Jerusalem" in content_all
            ), "ICS file must specify Asia/Jerusalem timezone"
...
async def test_pagination_fetches_all_data():
    ...
    async with aiohttp.ClientSession() as session:
        routes = await get_train_schedule(
            session, origin_id, dest_id, tomorrow, semaphore
        )
...
async def test_all_station_ids_valid():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for name, station_id in STATION_IDS.items():
            ...
            tasks.append(
                get_train_schedule(session, station_id, dest_id, tomorrow, semaphore)
            )
        ...
        for result, (name, station_id) in zip(results, STATION_IDS.items()):
            if isinstance(result, Exception):
                pytest.fail(
                    f"Station '{name}' with ID '{station_id}' failed API validation: {result}"
                )
